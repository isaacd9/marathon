...
class AppDefinitionTest extends MarathonSpec with Matchers {
  ...
  // Validation tests for `acceptedResourceRoles`.

  test("If `acceptedResourceRoles` is not provided, an app should be valid regardless of its residency.") {
    optional(AppDefinition.validAcceptedResourceRoles(isResident = false))(None).isSuccess shouldBe true
    optional(AppDefinition.validAcceptedResourceRoles(isResident = true))(None).isSuccess shouldBe true
  }

  test("If `acceptedResourceRoles` is provided, a non-resident app is valid only if it contains valid role names.") {
    val validate = AppDefinition.validAcceptedResourceRoles(isResident = false)
  ...
  test("""If `acceptedResourceRoles` is provided, a resident app is valid only if it is set to ["*"].""") {
    val validate = AppDefinition.validAcceptedResourceRoles(isResident = true)
